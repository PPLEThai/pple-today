...
- A user with more than one eligible role picks a บทบาท in the same prompt, preselected on a role the app is actually listed for, and the switch happens before the app opens. The roles are listed inline rather than in a Select: a Select opens through the portal host the dialog itself is using, and on iOS its dropdown lands under the dialog where no tap reaches it.
