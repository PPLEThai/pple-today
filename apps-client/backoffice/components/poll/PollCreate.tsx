...
export const PollCreate = (props: PollCreateProps) => {
  const [isOpen, setIsOpen] = useState(false)

  const topicQuery = reactQueryClient.useQuery('/admin/topics', { query: {} })
  // TODO: add the poll mutation
  const pollMutation = reactQueryClient.useMutation('post', '/admin/polls')
  ...
  const form = useForm<CreatePollFormSchema>({
    resolver: standardSchemaResolver(CreatePollFormSchema),
    defaultValues: {
      title: '',
      endAt: undefined,
      type: 'SINGLE_CHOICE',
  ...
  const onSubmit: SubmitHandler<CreatePollFormSchema> = async ({ ...data }) => {
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent asChild className="max-w-3xl">
        ...
        <DialogDescription className="text-sm text-base-text-medium leading-tight">
          สร้างแบบสอบถามสำหรับรวบรวมความคิดเห็นจากผู้ใช้งาน
        </DialogDescription>
